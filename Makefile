...
VERSION := $(shell git describe 2>/dev/null || echo 0.1.0)
ifeq ($(TEST_OS),)
TEST_OS = centos-7
endif
export TEST_OS
...
dist/index.js: $(wildcard src/*) package.json webpack.config.js $(patsubst %,dist/po.%.js,$(LINGUAS))
	NODE_ENV=$(NODE_ENV) npm run build

clean:
	rm -rf dist/
	rm -rf _install
	rm -f cockpit-$(PACKAGE_NAME).spec

install: dist/index.js
	mkdir -p $(DESTDIR)/usr/share/cockpit/$(PACKAGE_NAME)
	cp -r dist/* $(DESTDIR)/usr/share/cockpit/$(PACKAGE_NAME)
	mkdir -p $(DESTDIR)/usr/share/metainfo/
	cp io.zeelos.$(PACKAGE_NAME).metainfo.xml $(DESTDIR)/usr/share/metainfo/
...
test/common:
	git fetch --depth=1 https://github.com/cockpit-project/cockpit.git 170
