...
lwClientControllers.controller('ClientListCtrl', [
    ...
    function ClientListCtrl($scope, $http,$location) {
        ...
        leshan.get('/api/clients').fail(function(response, data) {
            $scope.$apply(function(){
                $scope.error = "Unable to get client list: " + response.status + " " + data;
...
lwClientControllers.controller('ClientDetailCtrl', [
    ...
    function($scope, $location, $routeParams, $http, lwResources,$filter) {
        ...
        leshan.get('/api/clients/' + $routeParams.clientId).fail(function(response, data) {
            $scope.$apply(function(){
                $scope.error = "Unable to get client " + $routeParams.clientId+" : "+ response.status + " " + data;
                console.error($scope.error);
            });
        }).done(function(data) {
            $scope.client = angular.fromJson(data);
