        ...
        link: function (scope, element, attrs) {
            ...
            scope.read = function() {
                ...
                leshan.get(uri, {format: format}).fail(function (response, data) {
                    errormessage = "Unable to read instance " + scope.instance.path + " for " + $routeParams.clientId + " : " + response.status + " " + data;
            ...
            scope.del = function() {
                ...
                leshan.request({method: 'DELETE', path: uri, body:{}})
                    .done(function(data) {
                        ...
                        errormessage = "Unable to delete instance " + scope.instance.path + " for "+ $routeParams.clientId + " : " + response.status +" "+ data;
                        dialog.open(errormessage);
                        console.error(errormessage);
                    });
            };
            ...
            scope.write = function () {
                ...
                modalInstance.result.then(function (instance) {
                    ...
                    leshan.request({method: 'PUT',
                        path: "/api/clients/" + $routeParams.clientId + scope.instance.path,
                        ...
                        .done(function(data) {
                            ...
                            errormessage = "Unable to write resource " + scope.instance.path + " for "+ $routeParams.clientId + " : " + response.status +" "+ data;
                            dialog.open(errormessage);
                            console.error(errormessage);
                        });
                });
            ...
            scope.startObserve = function() {
                var format = scope.settings.multi.format;
                var uri = "/api/clients/" + $routeParams.clientId + scope.instance.path+"/observe?" + "format="+format;

                leshan.post(uri, {}).fail(function (response, data) {
                    errormessage = "Unable to start observation on instance " + scope.instance.path + " for "+ $routeParams.clientId + " : " + response.status +" "+ data;
                    dialog.open(errormessage);
                    console.error(errormessage);
                }).done(function(data) {
                    scope.$apply(function() {
            ...
            scope.stopObserve = function() {
                ...
                leshan.request({method: 'DELETE', path: uri, body:{}, headers:{'Content-Type': 'application/json'}})
                    .done(function(data) {
                        scope.$apply(function() {
                        ...
                        errormessage = "Unable to stop observation on instance " + scope.instance.path + " for "+ $routeParams.clientId + " : " + response.status +" "+ data;
                        dialog.open(errormessage);
                        console.error(errormessage);
                   });
            };
