...
myModule.factory('lwResources',["$http", function($http) {
    ...
    var loadObjectDefinitions = function(callback) {
        if (objectDefs){
            ...
            leshan.get('/api/objectspecs').fail(function(response, data) {
                errormessage = "Unable to load object specfication : " + response.status +" "+ data;
                console.error(errormessage);
                callback([]);
            }).done(function(data) {
                if (data) {
                    objectDefs = angular.fromJson(data);
                    callback(objectDefs);
                }else{
                    callback([]);
                }
